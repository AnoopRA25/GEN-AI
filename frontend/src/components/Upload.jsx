...
import { CloudUpload, Image as ImageIcon, X, AlertCircle, CheckCircle2, FileImage } from 'lucide-react';
import axios from 'axios';
import toast from 'react-hot-toast';
import { DNALoader } from './ReactBitsComponents';
        ...
        {!preview ? (
          ...
          >
            ...
            <motion.div animate={drag ? { scale: 1.15 } : { scale: 1 }} transition={{ type:'spring', stiffness:300 }}
              ...
              style={{ background: 'rgba(212,168,83,0.08)', border: '1px solid rgba(212,168,83,0.22)', color: 'var(--accent-amber)' }}
            >
              <CloudUpload size={36} />
            </motion.div>
          ...
          >
            ...
            <div className="relative bg-black/40" style={{ maxHeight: 340 }}>
              ...
              {/* Scan overlay while loading — DNA helix loader */}
              {loading && (
                <div className="absolute inset-0 bg-black/70 backdrop-blur-sm flex flex-col items-center justify-center scan-effect">
                  ...
                  <AnimatePresence mode="wait">
                    <motion.p key={step}
                      ...
                      className="text-sm font-mono" style={{ color: 'var(--accent-amber)' }}
                    >
                      {STEPS[step]}
                    </motion.p>
                  </AnimatePresence>
                  <div className="mt-4 flex gap-1.5">
                    {STEPS.map((_, i) => (
                      <motion.div
                        ...
                        animate={{
                          background: i <= step ? '#d4a853' : 'rgba(212,168,83,0.15)',
              ...
              {!loading && (
                <button onClick={clear}
                  ...
                  style={{ background: 'rgba(0,0,0,0.7)', border: '1px solid rgba(212,168,83,0.15)' }}
                >
            ...
            <div className="p-5 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0"
                     style={{ background: 'rgba(212,168,83,0.08)', border: '1px solid rgba(212,168,83,0.20)' }}>
                  <ImageIcon size={16} style={{ color: 'var(--accent-amber)' }} />
                </div>
              ...
              <div className="flex gap-3 flex-shrink-0">
                ...
                <button onClick={analyse} disabled={loading} className="btn-primary text-sm px-6 py-2.5 flex items-center gap-2">
                  {loading ? <div className="loader-ring" style={{ width: '15px', height: '15px', borderWidth: '2px' }} /> : <CheckCircle2 size={15} />}
